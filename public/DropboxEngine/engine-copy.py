...
while True :
    ...
    time.sleep(delay)
    print("Listening")
    cur.execute("SELECT file_path, dropbox_token, fd.`id` FROM form_datas fd INNER JOIN forms f ON fd.`form_id` = f.`id` INNER JOIN users u ON f.`id_user` = u.`id` WHERE STATUS = 0")
    datas = cur.fetchall()
    for row in datas:
        local_path = "../../DynamicForm/public/"+row[0];
        dropbox_path = row[0];
        uploadFileDropbox(local_path, dropbox_path, row[1])
        cur.execute("UPDATE form_datas SET status = 1 WHERE id = " + str(row[2]))
        con.commit()
    con.rollback()